...
export class MergeStack<T> {
  private onChanges: Array<(conf: T) => void>;
   ...
   *
   * @template T
   * @param {T} obj - New object to be used for update
   */
  update(obj: T) {
    if (this.length === 0) {
      throw new Error('Empty config cannot be updated');
    }
    mergeWith(this.peek(), obj, this.customizer);
    this.runOnChanges();
  }
